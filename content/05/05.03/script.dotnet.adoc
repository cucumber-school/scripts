...
As you can see, the step definition implicitly takes a single argument shot::[8,"highlight stepdef parameters"] of type `Table`, which is a representation of a Data Table in SpecFlow. As it represents a table of persons, we can rename it to `presonsTable` shot::[9,"rename parameter"] 

This object has a rich API for using the tabular data. The `Rows` property of the table class can be used to access the data rows of the table -- all rows, except the header row. shot::[10,"split screen"] A particular cell value can be retrieved from a row by the header name. So, Lucy's location can be accessed by getting the "name" cell of the row at index 1. shot::[11,"Add cell value to an exception"]
...
Now we can easily iterate shot::[12,"close split, write the loop"] through the rows and turn them into instances of Person: 
