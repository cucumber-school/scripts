...
You can also turn the table into an array of arrays shot::[13,"Print out data table"], where the first row is used for the keys,shot::[14,"split IntelliJ screen horizontally, showing feature file at top and stepdefs below. Highlight first row"] and each following row is used for the map values.shot::[15, "highlight second and third rows"]
