...
Then we can shot::[18,"write defineWhereabouts()"] create a method annotated with @DataTableType so that Cucumber knows how to convert the table into a list of Whereabouts objects.
...
shot::[19a,"Run Cucumber"]
Let's run Cucumber to check that we're still green. And we are!

shot::[20,"hear_shout.feature data tables"]
That looks much nicer -  people positioned using a table in the feature file and shot::[21,"people_are_located_at()"] really clean code that creates and positions people according to the data.
