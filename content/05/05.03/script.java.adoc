...
Let's look at the two scenarios that illustrate the rule about range again shot::["show the range rule & scenarios"]. Notice how the steps shot::["compare the two scenarios"] that create the Sean, Lucy, and Larry are very similar.
...
[source,gherkin]
----
include::../code/java/05--step05-location-specified-in-data-table/shouty/src/test/resources/shouty/hear_shout.feature[lines=17..19]
----
...
As you can see, the step definition implicitly takes a single argument shot::["highlight stepdef parameters"], which as this comment explains is a DataTable. This object has a rich API for using the tabular data.

At its most basic, the table is just a two-dimensional array. So, Lucy's location can be accessed shot::["Print out a single cell value"] by getting the value from array cell (2, 1)
...
shot::["run Cucumber, highlight 'Lucy's location'"]

You can also turn the table into a List of Maps shot::["Print out data table"], where the first row is used for the map keys, and each following row is used for the map values.
...
shot::["run Cucumber, highlight data table output"]
...
shot::["run cucumber -- all green"]


With that done, we can update the other scenario shot::["update other scenario"] ...
...
... and delete our old step definition shot::["delete unused step def"], which is now unused.
...
Cucumber strips all the white space surrounding each cell shot::["show lining up of pipe characters"], so we can have a nice neat table in the Gherkin but still get clean values in the step definition underneath.

Notice we’ve still had to convert the location from a string to an integer shot::["Conversion in step def"], because Cucumber can’t know that’s the type of value in our table.
...
Then we can create a method annotated with @DataTableType so that Cucumber knows how to convert the table into a list of Whereabouts objects. shot::["write defineWhereabouts()"]
...
Now, if you declare your table parameter as a generic list shot::["modify stepdef"], Cucumber will automatically convert the table into a list of the generic type for you.
...
That looks much nicer - shot::["hear_shout.feature data tables"] people positioned using a table in the feature file and shot::["people_are_located_at()"] really clean code that creates and positions people according to the data.
