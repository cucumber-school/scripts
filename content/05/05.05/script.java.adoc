...
// FEDE: That gets explanied in the next lesson 5.06. Do you think we need to add some text that says that we explain it later?
// SEB: Yes, either add a vague explanation or say we'll see it on the next lesson. As someone who's trying to learn BDD and the difference between it and TDD, having a unit test added out of the blue raises questions.
...
One very basic move we could make to improve it would be to just extract a couple of temporary variables, one for the range rule shot::[15,"extract range temporary variable"] and one for the length rule. shot::[16,"extract length temporary variable"]
