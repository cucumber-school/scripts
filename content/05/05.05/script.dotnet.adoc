...
shot::[4.1,"select line with long text"] Our existing step definition handles that ugly step with the long message just fine, but the last outcome step is undefined shot::[4.2,"select Then"]. We could either add a new step definition, or parametrize "Larry should not hear a shout". Let's modify the existing step definition shot::[5,"Modify stepdef"]
...
OK, so we have a failing acceptance test.shot::[7, "select failing test in test explorer"] Let’s dive down into our solution and implement this new rule. It seems like the Network should be responsible for implementing this rule,  so let’s go to its unit tests. shot::[8,show NetworkTest] As we explain in more detail in the next lesson, we start by adding a new unit test to specify this extra responsibility. shot::[9, start creating new unit test]
...
Let’s run that test. shot::[12,"run tests"]shot::[13, "show the unit test results"] Good, it fails. Laura’s still getting the message at the moment. Now how are we going to implement this?
...
Let’s just run the tests to check. shot::[20,"run tests"] Great - everything’s still green.

As now we have everything passing again, we can tidy up the Gherkin to use a new piece of syntax we’ve been wanting to tell you about: a DocString. shot::[21,"show message length scenario in feature file"]
...
shot::[23,"highlight step"]
We have to add a new step definition too shot::[24,"add sean_shouts_the_following_message stepdef"]. It doesn't need a parameter in the Cucumber Expression shot::[25, "show expression"] -- the DocString gets passed as a string argument to the step definition automatically.shot::[26, "show param"]

Now we can fill out the rest of our new step definition. shot::[27]
...
Let's check that we're still green shot::[28,"run tests"] -- and we are!

These two step definitions do the same thing and they even have the same parameters. So we can keep just one of them and add the When attribute of the other to it. shot::[29, "move second When attribute up, delete second method"]
...
shot::[30,"run tests"] Having multiple Given, When or Then attributes on the same method is another way to handle alternates.

shot::[31,"switch back to feature file"]
