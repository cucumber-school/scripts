...
This two step definitions do the same and they even have the same parameters. So we can keep just one of them and add the When attribute of the other to it. shot::[30, "move second When attribute up, delete second method"]
...
shot::[31,"run tests"] Having multiple Given, When or Then attribute on the same method is another way to handle alternates.
