...
The way that we’ve specified this data is OK, but your product owner would prefer you to express it like this instead: shot::[1a,edit data table to have headings in the first column]
...
If you annotate the method parameter with the @Transpose annotation shot::[2,add Transpose annotation to stepdef], Cucumber will turn each row into a column before passing it to the step definition.
...
shot::[3a, show data tables in feature file]
Data tables are very useful for setting up data in Given steps, but you can also use them for specifying outcomes.

One rule that we’ve been implying but have never actually explored with an example is that people can hear more than one shout. So far we’ve only specified a single message, so let’s try  shot::[3b, write two-shouts scenario] writing a scenario where Sean shouts more than once:
...
Let's run Cucumber to generate the new step definition shot::[4, run Cucumber] and paste it into our StepDefinitions class shot::[5, paste snippet]
...
A List of List of String will do shot::[6,write code to create list-of-list-of-strings], so we can just iterate over Lucy’s messages and create a new single-item List for each row.
...
Now we can pass that list to the diff method shot::[7,write code to call diff] on the table of expected messages passed in from the Gherkin.
...
Let’s fix just one of these shot::[9,add an exclamation mark] so you can see how the diff output changes.
...
Let’s fix this last typo shot::[11,add another exclamation mark], and we should be green again.
