...
// ORIGINAL
Right, so those two scenarios we just left as just placeholders: the one where the listener is within range,shot::[3] and the one where the listener is out of range shot::[4] have been fleshed out, and are passing. Fantastic!

//ALTERNATIVE
Right, so those two scenarios we just left as just placeholders: the one where the listener is within range, and the one where the listener is out of range shot::[3, "Show output from Cucumber showing the in-range/out-of-range results"] are passing. Fantastic! If we look in the feature file, we can see how they have been fleshed out. shot::[4, "show the in-range/out-of-range scenarios in the feature file"]
...
shot::[5,Show all four scenarios in feature file]
...
We used the second scenario - Listener hears a different message shot::[6,Highlight second scenario] - to triangulate and force us to replace the hard-coded message output with a proper implementation. Now we have a domain model that uses a variable for the message, there's an insignificant chance of this behaviour regressing, so we can safely remove the second scenario. shot::[7,removes second scenario: Listener hears a different message]
