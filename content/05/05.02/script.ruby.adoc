...
The scenarios would still run green if we removed the steps that set the range of a shout shot::[20,"Highlight in last two scenarios"]shot::[20b] , because the range already has a default value. We're not going to, because since those scenarios are illustrating the rule that deals with the range of a shout, it's an essential part of context for anyone reading them.
