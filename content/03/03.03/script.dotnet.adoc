...
Because we've used Cucumber Expressions' built-in `{int}` parameter type, the value has been cast to a `int` data type for us automatically, so we can do maths with it if we want.shot::[5]
