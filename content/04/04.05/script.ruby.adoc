...
Now we can store Lucy in the hash. We’ll use her name as the key, hard-coding it for now.shot::[5]
...
Finally, where we check Lucy’s messages heard here in the assertion, we need to fetch her out of the hash. shot::[6]
...
Using your new found Cucumber expression skills from the last lesson, you’ll know that if we replace the word Lucy here with a 'word' expression,shot::[8] we’ll have the name passed into our step definition as an argument, here.shot::[9] Now we can use that as the key in the hash.shot::[10]
...
Let’s remove the old step definition,shot::[12] and fetch Sean from the hash here where he shouts his message.shot::[13]
