...
[source,java]
----
include::../code/java/00--step00-initial-commit/shouty/src/test/java/shouty/StepDefinitions.java[lines=10..34]
----
...
There are also a couple of new unit test classes in the Shouty package, one for the Network class,shot::[5, show existence of NetworkTest] and another one for the Person class.shot::[6, show existence of PersonTest]
...
shot::[7, run Cucumber & show output that demonstrates the unit tests are passing (run by clicking on the test folder and hitting run)]
The familiar `mvn test` command will run those unit tests as well as Cucumber.

shot::[8, "open the feature file"]

The first thing I notice coming back to the code is that the feature file is still talking about the distance between Lucy and Sean,shot::[9] but we haven’t actually implemented any behaviour around that yet.

[source,java]
----
include::../code/java/00--step00-initial-commit/shouty/src/test/resources/shouty/hear_shout.feature[]
----
