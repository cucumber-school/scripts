...
We can record what Sean shouts by storing it in an instance variable as the `When` step runs, shot::[21] and import it.shot::[22] This is a common pattern to use in Cucumber step definitions when you don’t want to repeat the same test data in different parts of a scenario. Now we can use that in the assertion check. shot::[23]
...
We also need to add a `messagesHeard` method to our Person class.shot::[24] Let's do that now, we'll just return an empty array for now.shot::[25]
