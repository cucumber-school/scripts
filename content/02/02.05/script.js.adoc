...
shot::[2]
To implement the first step, we need to create a couple of `Person` objects, with the specified distance between them. shot::[3] We can remove the `pending` status now,shot::[4] and this `console.log` statement,shot::[5] and write the implementation for the first step like this:
...
1) Scenario: Listener is within range # features/hear_shout.feature:2
   ✖ Given Lucy is located 15m from Sean # features/step_definitions/steps.js:4
       TypeError: this.lucy.moveTo is not a function
           at World.<anonymous> (/home/fedex/code/shouty/features/step_definitions/steps.js:7:13)
...
shot::[21]
In the `When` step, we're capturing Sean's message using the `{string}` pattern, and pass an argument to the function. We'll call it `message`.shot::[22]
...
And now the second step is green: shot::[26]
...
But before, we need to import `assertThat` shot::[29] and `is` shot::[30] from `hamjest`
...
shot::[32]
We also need to add a `messagesHeard` method to our Person class. Let’s do that now, we'll just return an empty array for now shot::[33]
