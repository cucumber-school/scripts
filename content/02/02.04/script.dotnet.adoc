...
shot::[20] We can put a breakpoint into the method and shot::[21] debug the scenario to see what's happening.

shot::[22] When the breakpoint hits, the value of the `distance` parameter is 15. 
...
If you're curious, that was caused by the shot::[23]`(.*)` expression in the parameter of the `Given` attribute. We’ll explain these expressions in detail in a future lesson.
