...
shot::[] This is the one where the listener is within range. shot::[] Given Lucy is located 15 metres for Sean, shot::[] When Sean shouts "free bagels at Sean's", shot::[], Then Lucy hears Sean's message.
...
If you don't have a "Test Explorer" in your Visual Studio, you can open it shot::[8] using the "Test Explorer" command from the "Build" menu.
...
The dialog can help us to create a new class and paste the selected snippets. shot::[14] We'll just call the class `Steps`. shot::[15]
...
If you're interested, that was caused by the shot::[24]`(.*)` expression in the parameter of the `Given` attribute. We’ll explain these expressions in detail in a future lesson.
