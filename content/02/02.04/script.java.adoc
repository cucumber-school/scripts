...
We’ll create a new class in the shouty package shot::[14] where we’ll paste those snippets. Make sure you create it under test and not main. shot::[15]
...
Notice that the number 15 does not appear anywhere in our code. The value is automatically passed from the Gherkin step to the step definition. If you're curious, that’s the shot::[27]`{int}` in the step definition pattern or _cucumber expression_. We’ll explain these patterns in detail in a future lesson.
