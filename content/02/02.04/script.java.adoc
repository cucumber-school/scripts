...
first scenario. Create a file called `hear_shout.feature` in the `shouty` directory
under `resources`.
...
You’ll see that Cucumber has found our feature file and read it back to us. We can see a summary of the results at the bottom - 3 steps,shot::[8] one scenario shot::[9] - all undefined.shot::[10]

Let’s run the scenario again from inside InteliJ.shot::[11]
Select the feature file shot::[12] and choose Run from the context menu.shot::[13]
This will give you similar output.shot::[14]
...
We now have a pending step shot::[20] and two skipped ones.shot::[21]

This means Cucumber found all our step definitions, and executed the
first one. But that first step definition throws a `PendingException`,shot::[22] which
...
If we run it again, shot::[25] we can see the number 15 pop up in the output. shot::[26]
...
Notice that the number 15 does not appear anywhere in our code. The value is automatically passed from the Gherkin step to the step definition. If you're curious, that’s the shot::[23]`{int}` in the step definition pattern or _cucumber expression_. We’ll explain these patterns in detail in a future lesson.
