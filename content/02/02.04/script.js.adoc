...
You’ll see Cucumber has found our feature file and read it back to us. We can see a summary of the results at the bottom: one scenario,shot::[10] three steps - all undefined.shot::[11]
...
We need to import the `Given`, shot::[15] `When` shot::[16] and `Then` shot::[17] keywords from the `cucumber` package.shot::[18]
...
This time the output is a little different. We now have a pending step shot::[20] and two skipped ones.shot::[21] This means Cucumber found all our step definitions, and executed the first one.
...
We'll rename the `int` parameter to something that better reflects its meaning. Let's call it `distance`.shot::[23]
