...
shot::[6]
Save the file, switch back to the command-prompt and run `npm test`. You’ll see Cucumber has found our feature file and read it back to us. We can see a summary of the results at the bottom: one scenario, three steps - all undefined.
...
shot::[12]
We need to import the `Given`, shot::[13] `When` shot::[14] and `Then` keywords from the `cucumber` package.
