...
shot::[6]
Save the file, switch back to the command-prompt and run `npm test`. You’ll see Cucumber has found our feature file and read it back to us. We can see a summary of the results at the bottom: one scenario,shot::[7] three steps - all undefined.shot::[8]
...
We need to import the `Given`, shot::[10] `When` shot::[11] and `Then` shot::[12] keywords from the `cucumber` package.shot::[13]
...
This time the output is a little different. We now have a pending step shot::[15] and two skipped ones.shot::[16] This means Cucumber found all our step definitions, and executed the first one.
...
We'll rename the `int` parameter to something that better reflects its meaning. Let's call it `distance`.shot::[18]
...
Notice that the number 15 does not appear anywhere in our code. The value is automatically passed from the Gherkin step to the step definition. If you're curious, that’s the shot::[21]`{int}` in the step definition pattern or _cucumber expression_. We’ll explain these patterns in detail in a future lesson.
