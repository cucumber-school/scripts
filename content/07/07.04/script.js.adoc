...
shot::[2,"Changes step to When Sean shouts a message containing the word "buy"]
[source, gherkin]
----
...
shot::[3,"Runs `cucumber --tags ~@todo`, copies / pastes step def snippet into steps file."]
...
shot::[4,"Copies code from step def below, then adds code above to create a test message containing the given word."]
...
shot::[5,"Runs `cucumber --tags ~@todo`."]
[source, bash]
----
npm exec cucumber-js -- --tags "not @todo"
----
...
shot::[6,"Changes the other two steps that mention the word “buy”. Runs `cucumber --tags ~@todo`. All green."]
...
shot::[7,"Changes the step from And Sean shouts "My bagels are yummy" to And Sean shouts a message"]
...
shot::[8,"Runs `cucumber --tags ~@todo`,  copies snippet. Copies shouting code from step def below, then adds code above to create a test message."]
...
shot::[9,"Runs `cucumber --tags ~@todo`."]
[source, bash]
----
npm exec cucumber-js -- --tags "not @todo"
----
...
shot::[10,"Changes two steps to that shout regular messages to use the new step"]
...
shot::[11,"Runs `cucumber --tags ~@todo`"]
...
shot::[12,"Changes the step to `And Sean shouts a long message`"]
...
shot::[13,"Runs `cucumber --tags ~@todo`"]

shot::[14,"Copies / pastes step def snippet into steps file"]

Again, we’ll tolerate the duplication for now, and just generate a long message here.

shot::[15,"Copies shouting code from step def below, then adds code above to create a 180 character test message."]
...
shot::[16,"Runs `cucumber --tags ~@todo`. All green."]
...
shot::[17,"Changes second rule “Long messages…” to “Over-long messages…”"]
...
shot::[18,"Changes step to `And Sean shouts an over-long message`"]
...
shot::[19,"Copies shouting code from step def below, then changes first line to create a 181-character test message."]
...
shot::[20,"Runs `cucumber --tags ~@todo`. All green."]

Again, we have another step we can replace with this more abstract one:

shot::[21,"Replace imperative step on line 22 with `Sean shouts an over-long message`"]
...
shot::[23,"Re-orders When steps so each type of step is grouped together."]
...
shot::[24,"Removes the first four When steps (remember to change And for When on the next step)"]
[source, gherkin]
----
...
shot::[25,"Replaces two steps with new step: `When Sean shouts 2 over-long messages`"]
...
shot::[26,"Runs `cucumber --tags ~@todo`. Copies step def snippet and pastes into steps file."]
...
shot::[27,"Copy / paste code from step def below. Add loop around the body of the code."]
...
shot::[29,"Replaces three steps with new step: `When Sean shouts 3 messages containing the word “buy”`"]
...
shot::[30,"Runs `cucumber --tags ~@todo`. Copies step def snippet and pastes into steps file."]
...
shot::[31,"Copy / paste code from step def below. Add loop around the body of the code."]
