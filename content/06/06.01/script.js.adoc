...
shot::[3, "type cucumber-js command in console, show output and highlight that it has only run the one scenario 'Message too long'"]
...
Let's use it to run the two scenarios with the text "range" in their name. shot::[4, "type cucumber-js command in console, show output and highlight that it has run two scenarios"]
...
We can accomplish the same thing by adding some configuration to the `cucumber.js` file, but we'll talk about that at the end of the lesson.
...
Let's add that line number to the cucumber-js command. shot::[10, "type cucumber-js command in console, show output and highlight that it has run two scenarios"]
...
When we run Cucumber, it picks up the property setting from the file and only runs the "range" scenarios. shot::[13, "type `cucumber-js` in console, show output and highlight that it has run the two scenarios"]
...
The property file is also picked up if you use npm to run Cucumber shot::[14, run `npm test` and then highlight that only two scenarios have been run]
