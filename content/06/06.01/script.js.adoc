...
Let's use it to run all scenarios with the text "range" in their name. shot::[4, "type mvn command in console, show output and highlight that it has run the two scenarios containing the word 'range'"]
...
We can accomplish the same thing by adding some configuration on the `cucumber.js` file, but we'll talk about that in the next lesson.
...
When we run `nmp test`, Cucumber picks up the property setting from the file and only runs the "range" scenarios. shot::[13, "type `mvn test` in console, show output and highlight that it has run the two scenarios containing the word 'range'"]
