...
Let's look at using cucumber.js to pass arguments to Cucumber. Open `cucumber.js` shot::[11] and add `default: --name range` shot::[12]
...
When we run the tests, it picks up the settings from cucumber.js and only runs the "range" scenarios. shot::[13, "type `npm test` in console, show output and highlight that it has run the two scenarios"]
