...
We can accomplish the same thing by adding some configuration on the `cucumber.yml` file, but we'll talk about that in the next lesson.
...
We can use that line number when we run Cucumber. shot::[7, "add entry to property file"]
...
shot::[8, "type mvn command in console, show output and highlight that it has only run the scenario called 'Message too long'"]
...
Let's add that line number to the Maven command.shot::[10, "type mvn command in console, show output and highlight that it has run two scenarios - 'Two shouts' and 'Message too long'"]
...
The property file is also picked up if you run Cucumber from within your IDE shot::[14, use IntelliJ to run all scenarios in `hear_shout.feature` and then highlight that only the range scenarios have been run]
