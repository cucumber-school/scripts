...
shot::[3, "type bundle exec command in console, show output and highlight that it has only run the one scenario 'Message too long'"]
...
We can also use the abbreviated form of `--name`, `-n` shot::[5]
...
We can use that line number when we run Cucumber. shot::[7, "use line number on comand line"]
...
shot::[8, "type bundle exec command in console, show output and highlight that it has only run the scenario called 'Message too long'"]
...
Let's add that line number when we run Cucumber. shot::[10, "type bundle exec command in console, show output and highlight that it has run two scenarios - 'Two shouts' and 'Message too long'"]
...
Let's look at using a property file to set the system properties. Create a property file called `cucumber.yml` in the root path of the project shot::[11] and add `default: --name range` shot::[12]

When we run Cucumber, it picks up the property setting from the file and only runs the "range" scenarios. shot::[13, "type `mvn test` in console, show output and highlight that it has run the two scenarios containing the word 'range'"]
...
The property file is also picked up if you run Cucumber from within your IDE shot::[14, use an IDE to run all scenarios in `hear_shout.feature` and then highlight that only the range scenarios have been run]
