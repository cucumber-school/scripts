...
Now let's run Maven again using the `rerun` plugin writing its output to a file whose name is preceded by the `@` (at) sign. Let's call it `@rerun.txt`. shot::[17, run maven from console]
