...
SpecFlow is first and foremost a tool that facilitates a common understanding between people on a project. To be able to express the domain requirements of project implemented specifically for a non-English speaking local market, translating everything to English might be a barrier. In this case it is better to describe the requirements in the spoken language the stakeholders speak. 

SpecFlow supports over 70 different languages, thanks to contributions from people from all over the world. 

To see the translation of the Gherkin keywords for a particular language, check out the https://cucumber.io/docs/gherkin/languages/[Gherkin syntax documentation]. This is for example how the keywords translate to Hungarian. shot::[2, "scroll to Hungarian"]

Let's see how someone would create a feature file written in a different spoken language. I will do this for Hungarian language, but you can choose another language you speak. 
...
Now we can start writing our feature file. Remember, it should start with a feature keyword. The auto-complete feature of the Visual Studio integration is for your help. shot::[5, "invoke auto-complete, choose 'Jellemző'"] It shows what keyword should I use. shot::[6, "finish line"]
...
If we build the project, the first two step appears to be defined now. shot::[12, "build and see the feature file coloring changing"]
...
When we build the project, shot::[21, "build, show errors"] we got a lots of errors from our English feature file. This is because we changed the default and this file does not have a language setting on the top. shot::[22, "show eng feature file, fix by adding #language: en-US"] But the new file works without the language setting now... shot::[23, "switch to hun, remove language setting"]

In Chapter 3 we talked about step definition parameters and that SpecFlow helps you to convert them to a .NET data type. SpecFlow performs these conversions using the feature file language. So for example if you would include a fraction number that needs to be converted to double, SpecFlow will expect the decimal separator character used in the language of your feature file. In Hungarian language for example, they use comma as a decimal separator instead of a dot. This is also important for date conversions. 

This default behavior can be changed by explicitly declaring what culture setting should SpecFlow use for conversions. For example we can force the conversions to be made using US English culture even in Hungarian feature files. shot::[24, "change config file to"]
