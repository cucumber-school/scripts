...
Tags can be used for other purposes as well. If you have lots of scenarios it can be time-consuming to run them all every time. For example, you can tag a few of them with @smoke and run only those before you check in code to source control. shot::[9, tag first, second and third scenario with @smoke]
...
Running just the smoke tests will give you a certain level of confidence that nothing is broken without having to run them all. shot::[10, "run Cucumber, then highlight that three scenarios were run"]
...
Some teams also use tags to reference external documents, for example, tickets in an issue tracker or planning tool. Let’s pretend we are using an issue tracker while working on Shouty and all the behaviour we built so far is related to the issue number 11. We could tag the whole feature file with this single line at the top. shot::[11, tag feature with @SHOUTY-11]
...
All the scenarios within that file now inherit that tag, so if we change the tag expression in cucumber.js, Cucumber will run all the scenarios in the feature file. shot::[12, "run Cucumber, then show that all scenarios were run"]


You can use more complex tag expressions to select the scenarios you want to run. For example, you could use a tag expression to exclude all the scenarios tagged as @slow. shot::[13, "tag last two scenarios with @slow""]
...
Then rewrite the tag expression in cucumber.js using the `not` keyword shot::[14, "modify Cucumber.JS]
