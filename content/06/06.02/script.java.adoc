...
First, let's delete the contents of cucumber.properties. shot::[3]
...
Now we can run only the scenarios tagged with focus - there should be only one... shot::[7, "run RunCucumberTest.java from IntelliJ, then highlight that only 'Listener is out of range' scenario was run"]
...
Some teams also use tags to reference external documents, for example, tickets in an issue tracker or planning tool. Let’s pretend we are using an issue tracker while working on Shouty and all the behaviour we built so far is related to the issue number 11. We could tag the whole feature file with this single line at the top. shot::[10, tag feature with @SHOUTY-11]
...
All the scenarios within that file now inherit that tag, so if we change the tag expression in @CucumberOptions, shot::[10, change CucumberOptions filter to @SHOUTY-11]
...
Cucumber will run all the scenarios in the feature file. shot::[11, "run RunCucumberTest.java from IntelliJ, then show that all scenarios were run"]

You can use more complex tag expressions to select the scenarios you want to run. For example, you could use a tag expression to exclude all the scenarios tagged as @slow. shot::[12, "tag last two scenarios with @slow""]
...
Then rewrite the tag expression in CucumberOptions using the `not` keyword shot::[13, "modify CucumberOptions]
...
Now when you run Cucumber, the "@slow" scenarios won't be run. shot::[14, "run RunCucumberTest.java from IntelliJ, then show that the slow scenarios were not run"]

Let's tidy up be removing the tag filter from CucumberOptions. shot::[]
