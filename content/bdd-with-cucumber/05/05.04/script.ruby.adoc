...
[source,gherkin]
----
include::../code/ruby/10-changed-other-scenario-and-deleted-unused-stepdef/features/hear_shout.feature[lines=13..31]
----
...
Let's run Cucumber to generate the new step definition shot::[7, run Cucumber] and paste it into our step definitions file  shot::[8, paste snippet] shot::[9]
...
An array of arrays will do shot::[10,write code to create list-of-list-of-strings], so we can just iterate over Lucy’s messages and create a new single-item array for each row.shot::[11]
...
Now we can pass that array to the diff method on the table of expected messages passed in from the Gherkin. shot::[12, write code to call diff]
...
Let’s fix just one of these shot::[15, add exclamation mark] so you can see how the diff output changes.
...
The matching bagels! line no longer has a minus, and for the mismatched row, the expected value still has a minus, and the actual value has a plus.
