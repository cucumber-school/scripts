...
We're going to need the `ShoutyWorld` to be available shot::[] to us in this context so that we have access to the `network` and `people` fields. Picocontainer's dependency injection pattern works in classes with the `@ParameterType` annotation just the same as it does in a step definition class. If you're using Spring for dependency injection, you'll need to add the `@ScenarioScope` annotation to the class. See https://github.com/cucumber/cucumber-jvm/tree/main/spring[Cucumber-JVM's Spring documentation] for details.
...
Great! We've added our first parameter type. Now we want to use it everywhere we can. shot::[1, "Fast montage of changing all the steps from {word} to {person}"]
