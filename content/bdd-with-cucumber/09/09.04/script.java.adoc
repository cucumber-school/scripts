...
We won't be using just a `{word}` anymore shot::[], we'll use a `{person}` shot::[], and instead of receiving a plain `String` shot::[] we'll receive an instance of `Person`. Now we can use shot::[] this `Person` object in the body of our step.

[source,java]
----
include::../code/java/08-use-parameter-type-in-one-step-fails-/shouty/src/test/java/shouty/StepDefinitions.java[lines=50..53]
----
