...
And run Cucumber shot::[4] to check everything's still working. Great.
...
We could continue to move code from our step definitions into the support layer. For example, we could write a support method for generating messages of a certain length shot::[5, "Highlight lines 15-16"], 
...
or a method for asserting that a person has heard certain messages. shot::[6, "Highlight lines 22-24"]
...
Ideally, each step definition shot::[7] should only contain one or two lines that delegate to your support code. 
