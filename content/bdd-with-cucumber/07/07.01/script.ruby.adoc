...
[source,gherkin]
----
include::../code/ruby/01-implement-premium-accounts-feature-badly-/features/premium_shouts.feature[]
----
...
Let’s see if we can gleam the rules from reading the scenario carefully. Sean starts out with 30 shot::[8] credits and ends up with 11, but why?shot::[9]
...
[source,javascript]
----
include::../code/ruby/01-implement-premium-accounts-feature-badly-/lib/shouty.rb[lines=24..50]
----

OK, so we have a `deduct_credits` method here that seems to encapsulate the rules.shot::[11] It looks like over-long messages - messages that are not short enough - cost two credits,shot::[12] and each time the word “buy” is mentioned, we deduct 5 credits.shot::[13]
