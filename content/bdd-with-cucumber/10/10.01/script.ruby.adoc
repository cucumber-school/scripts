...
We have it marked with a `@todo` tag on it,shot::[1] but those scenarios are filtered out in our default `bundle exec rake` command.
...
We're going to start work on the scenario now, so let's change that tag to a `@wip`shot::[2] to indicate that it's "Work In Progress":
...
Now when we run our regular `bundle exec rake` command, we should see the failing scenario: shot::[3]
...
You might be worried about losing this bug ID. We could keep it in a comment shot::[8], a tag shot::[9], or in the description of the Scenario.
...
So let's do that. shot::[10, write the bug number in the scenario description, run tests]
...
We think the values in the example could be changed to make it a little more expressive. If we start Sean off with 100 credits,shot::[11] and end him with 95,shot::[12] it more clearly illustrates the rule that only five credits should be deducted. shot::[13, run tests]

[source,gherkin]
----
include::../code/ruby/03-changed-values-in-scenario/features/premium_shouts.feature[lines=19..25]
----
